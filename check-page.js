const STRAPI_URL = 'http://localhost:1337';
const STRAPI_API_TOKEN = '';

const documentId = 'r3v3t8tmi7acld61xq3oz2lr';

async function checkPage() {
  try {
    console.log('🔍 Checking page with documentId:', documentId);
    
    // Try by documentId
    const response = await fetch(`${STRAPI_URL}/api/pages/${documentId}?populate=*`, {
      headers: {
        'Authorization': `Bearer ${STRAPI_API_TOKEN}`
      }
    });
    
    const data = await response.json();
    console.log('\n📥 Response status:', response.status);
    console.log('📥 Response data:', JSON.stringify(data, null, 2));
    
    if (response.ok && data.data) {
      console.log('\n✅ Page found!');
      console.log('   - ID:', data.data.id);
      console.log('   - documentId:', data.data.documentId);
      console.log('   - title:', data.data.title);
      console.log('   - pageType:', data.data.pageType);
      console.log('   - userId:', data.data.userId);
      console.log('   - template:', data.data.template);
    } else {
      console.log('\n❌ Page not found or error');
    }
  } catch (error) {
    console.error('❌ Error:', error.message);
  }
}

checkPage();
