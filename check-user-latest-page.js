// Check the latest page created by the user
const STRAPI_URL = 'http://localhost:1337';
const STRAPI_API_TOKEN = '';

async function checkLatestPage() {
	console.log('🔍 Checking latest pages...\n');
	
	try {
		// Get latest 5 pages
		const response = await fetch(`${STRAPI_URL}/api/pages?pagination[limit]=5&sort=createdAt:desc`, {
			headers: {
				'Authorization': `Bearer ${STRAPI_API_TOKEN}`
			}
		});
		
		const result = await response.json();
		
		console.log(`✅ Found ${result.data.length} pages\n`);
		
		result.data.forEach((page, index) => {
			console.log(`📄 Page ${index + 1}:`);
			console.log(`   - ID: ${page.id}`);
			console.log(`   - DocumentID: ${page.documentId}`);
			console.log(`   - Title: ${page.title}`);
			console.log(`   - Slug: ${page.slug}`);
			console.log(`   - PageType: ${page.pageType}`);
			console.log(`   - userId: ${page.userId || '❌ EMPTY'}`);
			console.log(`   - metadata.createdByUserId: ${page.metadata?.createdByUserId || '❌ EMPTY'}`);
			console.log(`   - Created: ${page.createdAt}`);
			console.log('');
		});
		
	} catch (error) {
		console.error('❌ Error:', error.message);
	}
}

checkLatestPage();
